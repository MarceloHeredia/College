...
func procPegaB(minhaPos GridPosition, ultMovFugitivo chan GridPosition, meuUltMov chan GridPosition, posIniFoge GridPosition){
	posFugitivo := posIniFoge
	for {
		if len(ultMovFugitivo) > 0{
			posFugitivo = <- ultMovFugitivo	//se a posicao do fugitivo mudar le do canal
		}
...
encerraPegaB: end <- true
}

func procFogeB(minhaPos GridPosition, ultMovPegador chan GridPosition, meuUltMov chan GridPosition, posIniPega GridPosition){
	posPegador := posIniPega
	for{

		if len(ultMovPegador) > 0{//se tiver mudança na posicao do jogador
			posPegador = <- ultMovPegador  //executa leitura
		}
		//ao ler, eh sua vez
...
func main(){
	...
	//Variante B
	go procPegaB(posPega, movFoge, movPega, posFoge)//alem dos dados anteriores, adiciona a posicao inicial do foge
	go procFogeB(posFoge, movPega, movFoge, posPega) //mesma coisa
